...
from parameter import rml_file_name_tor_2400 as rml_file_name
...
# np.genfromtxt(filename, dtype=float, delimiter='\t', names=True,skip_header=1)
ppa = PostProcessAnalyzed()

from parameter import order as diff_order
varying_var = diff_order
varying_var_n = 'diff order'
varying_var_unit = ''
...
fig.suptitle("NAP-LEEM, 2400 l/mm grating")
...
ax.axhline(y=70, color='black', linestyle='dashed', label='70 meV')
ax.axhline(y=140, color='grey', linestyle='dashed', label='140 meV')
