...
from parameter import SlitSize, cff
from parameter import round_rp, round_flux
...
for ind, es in enumerate(varying_var):
    ax.plot(energy_flux,flux[ss*ind:ss*(ind+1)], colors[ind], label=f'{varying_var_n} {es} {varying_var_unit}' )
...
for ind, es in enumerate(varying_var):
    ax.plot(energy_rp,bw[ss*ind:ss*(ind+1)],colors[ind],label=f'{varying_var_n} {es} {varying_var_unit}')
...
for ind, es in enumerate(varying_var):
    ...
    ax.plot(energy_rp, rp, colors[ind], label=f'{varying_var_n} {es} {varying_var_unit}')
...
ax = axs[2,0]
for ind, es in enumerate(varying_var):
    ax.plot(energy_rp,focx[ss*ind:ss*(ind+1)],colors[ind],label=f'{varying_var_n} {es} {varying_var_unit}')
...
ax = axs[2,1]
for ind, es in enumerate(varying_var):
    ax.plot(energy_rp,focy[ss*ind:ss*(ind+1)],colors[ind],label=f'{varying_var_n} {es} {varying_var_unit}')
